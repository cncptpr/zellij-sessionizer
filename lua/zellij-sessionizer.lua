...
local function append_all_paths(list, path)
  ...
  if not is_dir(base_path) then
    print(ANSI.YELLOW .. "Warning:" .. ANSI.RESET .. " Directory not found: " .. base_path)
    return false
  end
...
local function main(paths)
  ...
  for _, path in ipairs(paths) do
    if not append_all_paths(candidates, path) then
      print(ANSI.YELLOW .. "Warning:" .. ANSI.RESET .. " Directory not found: " .. path)
    end
  end
